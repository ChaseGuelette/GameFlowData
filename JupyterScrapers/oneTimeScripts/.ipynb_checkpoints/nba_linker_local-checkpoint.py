"""
NBA Data Linker - Local Processing Version
==========================================
Downloads data locally, processes in pandas, uploads results.

Steps:
1. python nba_linker_local.py download   - Pull tables to CSV
2. python nba_linker_local.py process    - Match IDs locally  
3. python nba_linker_local.py upload     - Push results back
...
def process_local():
    """Process all matching locally in pandas."""
    print("Loading CSVs...")
    ...
    # ========================================
    # Build props game lookup: (home, away, date) -> game_id
    # ========================================
    print("\nBuilding props game lookup...")
    ...
    for _, row in home_games.iterrows():
        ...
        if away_team:
            ...
            key = (row['home_team_norm'], away_norm, row['game_date'])
            props_game_lookup[key] = row['game_id']
    
    print(f"  {len(props_game_lookup):,} home/away/date combinations")
    ...
    if len(unlinked_lines) > 0:
        ...
        def match_game_line(row):
            ...
            if match:
                return pd.Series({'nba_game_id': match[0], 'nba_home_team_id': match[1], 'nba_away_team_id': match[2]})
        ...
        matched_lines = unlinked_lines[unlinked_lines['nba_game_id'].notna()][['staging_id', 'nba_game_id', 'nba_home_team_id', 'nba_away_team_id']]
    ...
    # ========================================
    # MATCH PLAYER PROPS - GAMES
    # ========================================
    print("\n" + "="*60)
    print("MATCHING PLAYER PROPS -> GAMES")
    print("="*60)
    ...
    if len(unlinked_props) > 0:
        ...
        def match_prop_game(row):
            key = (row['home_team_norm'], row['away_team_norm'], row['game_date'])
            return props_game_lookup.get(key)
        
        tqdm.pandas(desc="Matching props to games")
        unlinked_props['matched_game_id'] = unlinked_props.progress_apply(match_prop_game, axis=1)
        ...
        print(f"Matched: {len(matched_props):,}")
    ...
    if len(needs_player) > 0:
        ...
        if len(unmatched) > 0:
            ...
            print(f"  Format: api_name,player_id")
    ...
    print("  3. Re-run 'python nba_linker_local.py process' to apply new mappings")
    print("  4. Run 'python nba_linker_local.py upload' to push to database")
...
def main():
    parser = argparse.ArgumentParser(description='NBA Data Linker - Local Processing')
    parser.add_argument('command', choices=['download', 'process', 'upload', 'all', 'init'],
                        help='Command to run')
