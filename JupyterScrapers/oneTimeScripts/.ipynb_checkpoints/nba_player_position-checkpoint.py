...
def get_position_group(pos):
    ...
    pos = pos.upper()
    if 'GUARD' in pos: return 'Guard'
    if 'FORWARD' in pos and 'CENTER' in pos: return 'Big'
    if 'FORWARD' in pos: return 'Forward'
    if 'CENTER' in pos: return 'Big'
    return 'Other'
...
def update_player_positions():
    ...
    for i, row in players.iterrows():
        ...
        try:
            print(f"[{i+1}/{len(players)}] Fetching info for player {player_id}...", end=" ", flush=True)
