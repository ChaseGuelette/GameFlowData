...
print(
    f"OVER bets:  {len(over_bets):,} | Wins: {over_wins} | Losses: {over_losses} | Hit rate: {over_wins / (over_wins + over_losses):.1%}"
)
print(
    f"UNDER bets: {len(under_bets):,} | Wins: {under_wins} | Losses: {under_losses} | Hit rate: {under_wins / (under_wins + under_losses):.1%}"
)
...
for stat in ["pts", "reb", "ast"]:
    ...
    over_roi = (
        stat_over["profit"].sum() / stat_over[stat_over["outcome"] != "push"]["stake"].sum()
        if len(stat_over) > 0
        else 0
    )
    under_roi = (
        stat_under["profit"].sum() / stat_under[stat_under["outcome"] != "push"]["stake"].sum()
        if len(stat_under) > 0
        else 0
    )
