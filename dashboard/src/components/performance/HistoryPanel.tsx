...
export function HistoryPanel() {
  ...
  const modelHistory = useModelHistory(startDate, endDate, activeTab === 'model_history')
  const myBetsQuery = useMyBets(startDate, endDate, activeTab === 'my_bets')
  const dfsEntriesQuery = useDfsEntries(startDate, endDate, activeTab === 'dfs_entries')
  const mlbModelQuery = useMLBModelHistory(startDate, endDate, activeTab === 'mlb_model')
    ...
    <section>
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-slate-50">Bet History</h2>
        ...
        <>
          {myBetsQuery.isLoading ? (
            ...
            <div className="bg-red-900/30 border border-red-700 rounded-lg px-5 py-4 text-sm text-red-300">
              Failed to load your bets: {myBetsQuery.error instanceof Error ? myBetsQuery.error.message : 'Unknown error'}
            </div>
          ) : myBets.length === 0 ? (
            <div className="flex items-center justify-center py-16">
      ...
      {activeTab === 'model_history' && (
        <>
          {modelHistory.isLoading ? (
            ...
            <div className="bg-red-900/30 border border-red-700 rounded-lg px-5 py-4 text-sm text-red-300">
              Failed to load model history: {modelHistory.error instanceof Error ? modelHistory.error.message : 'Unknown error'}
            </div>
          ) : (
            <>
      ...
      {activeTab === 'mlb_model' && (
        <>
          {mlbModelQuery.isLoading ? (
            ...
            <div className="bg-red-900/30 border border-red-700 rounded-lg px-5 py-4 text-sm text-red-300">
              Failed to load MLB history: {mlbModelQuery.error instanceof Error ? mlbModelQuery.error.message : 'Unknown error'}
            </div>
          ) : (
            <>
      ...
      {activeTab === 'dfs_entries' && (
        <>
          ...
          ) : dfsEntriesQuery.error ? (
            <div className="bg-red-900/30 border border-red-700 rounded-lg px-5 py-4 text-sm text-red-300">
              Failed to load DFS history: {dfsEntriesQuery.error instanceof Error ? dfsEntriesQuery.error.message : 'Unknown error'}
            </div>
          ) : (
            <>
