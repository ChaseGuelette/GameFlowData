...
- [[handoff-015]] — April 20, 2026 — MLB Feature Gap Sprint: weather features (Open-Meteo), L/R park factors, all 4 models retrained, direction restrictions removed, new BL configs promoted (+28% ROI hits, +24% pitcher K, +9.5% RBIs).
- [[handoff-016]] — April 20, 2026 — Kalshi live trading post-mortem: 8-fix overhaul (resolution decoupling, sport gate, edge sanity cap, shared $200 exposure, trade approval flow with dashboard UI, morning resolution job, game start time). Railway env vars deployed. 21 bets partially resolved but PnL garbage from null fill_price.
- [[handoff-017]] — April 21, 2026 — Re-enable Kalshi live trading (MLB only). HWM drawdown, dynamic 70% exposure cap, Discord approval embed, TradeApprovalPanel crash fix, nixpacks libz.so.1 fix (/app/lib → /opt/lib). Railway env vars deployed. Pending: verify Railway rebuild + set Vercel service role key.
- [[handoff-018]] — April 21, 2026 — Two-track non-sports arb system. Fuzzy matches now queue to `verified_market_links` for human review; only structured + approved pairs flow to `arb_opportunities`. Queue tab + approve/reject UI added to arb-scanner dashboard. Step 9.7 completed.
- [[handoff-019]] — April 21, 2026 — MLB paper trader double-bet fix (dedup by player+stat, ON CONFLICT 4-col→3-col). Max daily bet caps removed from all stats. Bookmaker exclusion list added to live runner + backtest sweep (novig, betonlineag, DFS platforms). Volume anomaly root cause confirmed: sportsbook market expansion, not a code bug. Migration 028 applied.
- [[handoff-020]] — April 21, 2026 — MLB paper trader deep dive: batter_rbis confirmed broken (-$13k, no profitable config on Apr 2026). Non-RBI stats +$14k. Kalshi line tiebreaker implemented. 3% bet cap added. Extended market scraping enabled (HRR lines). Combined backtest crash fixed.
