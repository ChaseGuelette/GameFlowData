...
| 1.3 | Train batter hits/total_bases models | completed | 1.1 | `batter_hits` promoted (tau=0.9, z_max=0.25, mw=0.5, edge=10%, +28.1% ROI). `batter_rbis` promoted (no BL, edge=12%, +9.5% ROI). DROPPED: `batter_total_bases` (0/540 profitable), `batter_runs_scored` (trivial edge), `batter_home_runs` (no edge). Configs updated Apr 20 after weather+L/R sprint. |
...
| 2.6.3 | Track Record page | completed | 2.6.1, 2.6.2 | Originally at `/track-record`, **later merged into `/performance` as the 4th "Record" tab** (My Bets \| Props \| DFS \| Record). All 5 components (MonthlyGrid, ModelMetrics, CsvUpload, ManualBetForm, DailyBreakdown) are live in `performance/page.tsx`. Source toggle (My Bets / Paper / Combined), KPI banner, edge accuracy buckets, streaks. |
...
| 7.6 | Dashboard prediction markets page | completed | 7.3, 7.4 | `/prediction-markets` route, sortable/filterable table, detail modal, countdown, fee breakdown. **Note**: Route later removed — standalone UI page deleted, backend infra remains. Edge data accessible via bot-tracker. |
...
| 7.13 | Sportsbook line alignment + star filter | completed | 7.10 | Fixed live trader SQL bug (missing sportsbook_consensus_line), replaced 3% tiebreaker with 8% SB-alignment dedup, star-hitter filter (yes_price >= 72 blocks NO on 1+ hits for stars), approval panel shows SB line + mismatch warning. See [[handoff-041]]. |
| 7.14 | Production stability fixes + UTC/ET timezone systemic fix | completed | 7.12 | (1) MLB edge refresh CTE: replaced `pd.read_sql(...params=...)` with `conn.execute(text().bindparams())` — SQLAlchemy 2.0 named param binding failure (`psycopg2 f405`). (2) Kalshi orderbook parallelization: `ThreadPoolExecutor(max_workers=10)` — was 16–17 min/run on 10-min schedule, now ~1 min. (3) Global `numpy.int64` psycopg2 adapter in `src/db/client.py`. (4) UTC/ET timezone systemic fix: 11 callsites across 6 files changed from `snapshot_time::date` to `(snapshot_time AT TIME ZONE 'America/New_York')::date` — root cause of 19 failed live trades on Apr 22 (evening ET games roll to next UTC day after 8 PM ET). Files fixed: `market_matcher.py` (4), `kalshi_live_trader.py` (2), `kalshi_paper_trader.py` (1), `arb_paper_trader.py` (1), `kalshi_edge.py` (1), `kalshi_refresh_job.py` (2). Zero bare `::date` casts remain. See [[handoff-043]]. |
| 7.15 | Fill polling + Discord alerts + daily summary | completed | 7.12 | (1) `reconcile_fills(target_date=None)` — date bug fixed, now queries all pending orders across all game dates. (2) `src/orchestration/kalshi_pending_fills_job.py` created — polls every 5 min, exits early if no pending, sends "filled" Discord embed. (3) Discord "placed" alert in `kalshi_execute_approved_job.py`. (4) "filled" + "daily_summary" embed builders added to `alerts.py`. (5) `_send_live_trader_summary()` in `kalshi_daily_summary_job.py` — yesterday W/L/PnL + all-time total. See [[handoff-044]]. |
