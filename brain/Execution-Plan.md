...
| 9.4 | Cross-platform market matcher: non-sports | in_progress | 9.2 | `market_matcher.py` extended with `match_non_sports_markets()` (SequenceMatcher ≥ 0.80 text similarity). `arb_scan_job.py` + `scheduler.py` wired with `--include-non-sports` flag (off by default, enabled on hourly all-categories job). Alerts formatting updated for game markets ("BOS vs MIA [MONEYLINE]") and non-sports ("[POLITICS] question text"). 694 tests pass. Session 13: Kalshi non-sports scraping infrastructure complete — `KALSHI_NON_SPORTS_SERIES` (9 series: KXGDP, KXFED, KXCPI, KXBTC, KXBTCD, KXETH, KXETHD, KXDOGE, KXXRP) added to `kalshi_utils.py`, `scrape_non_sports_and_store()` added to `kalshi_market_scraper.py`, `kalshi_nonsports_refresh_job.py` created, scheduler job runs every 10 min 11AM-11PM ET, `make_kalshi_markets_sport_nullable` migration applied, keyword pre-filter in `match_non_sports_markets()` (27,711 → ~2,900 Poly markets). 1,044 non-sports markets in DB. 0 matches produced (wording mismatch at 0.80 threshold). Session 35: Replaced SequenceMatcher approach with structured field extraction — new `src/arbitrage/non_sports_extractor.py` extracts (price, direction, period) deterministically from Kalshi ticker + Polymarket text. Fallback to SequenceMatcher ≥ 0.80 when extraction fails. Expected match count: ~10-100 (down from 2,447 false positives). **Pending**: dry-run validation (`arb_scan_job.py --include-non-sports --dry-run`) + Railway deploy. |
