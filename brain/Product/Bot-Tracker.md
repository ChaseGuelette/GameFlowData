...
- **Failed Orders section** (Session 53): "Failed Orders (last 24h)" panel rendered below pending approval panel when `failedTrades.length > 0`. Red border/background. Shows same fields as pending card. Per-row **Retry** button POSTs `action:'retry'` to `/api/kalshi/approve`, re-entering the failed trade with a fresh 30-min expiry. Discord fires red embed (0xFF0000) for each failed fill via `send_kalshi_trade_failure_alert_sync()` in `kalshi_execute_approved_job.py`.
