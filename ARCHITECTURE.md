...
- **Contextual Features:**
    ...
    - **Position-Matched Injury Opportunity (B5, added 2026-03-25):** `team_out_same_pos_count`, `team_out_same_pos_min_sum`, `team_out_same_pos_usg_sum`, `team_out_same_pos_starter_sum` — aggregates for OUT teammates in the same position group (G/W/B) as the player being predicted. `starter_sum` = sum of `games_started_l5 / 5.0` for each out player, distinguishing "starter at your position is out" (high value, e.g., Ant Edwards = 1.0) from "deep bench player is out" (low value). Computed in bulk training via grouped aggregation on `_load_injury_features_bulk()` per-player rows, and at inference time via a position-filtered SQL query in `_get_injury_context()`.
...
- **Predictions:** Upserted to `daily_predictions` table via `psycopg2.extras.execute_values` with `ON CONFLICT DO UPDATE`. Stores quantiles, edges, implied probabilities, prop line info, `prop_line` (inference-time prop line value for drift detection), `sanity_flag` (warning reasons for suspect under predictions), and position-matched injury features (`feat_team_out_same_pos_*`).
...
- **MLB jobs (2026-03-25):** `mlb_lines_job.py` orchestrates MLB props + game lines scraping via `mlb_daily_player_props_scraper.py` and `mlb_daily_game_lines_scraper.py`, then runs `mlb_linker incremental`. Supports `--live`, `--props-only`, `--parallel`. Schedule: full lines at 12 PM/5 PM ET, props-only at 1 PM/6 PM ET, inference at 1:30 PM/6:30 PM ET, daily stats at 10 AM ET (with 10:30 AM retry).
...
│   │   ├── api/games/route.ts    # NBA/MLB schedule proxy (fallback game list, sport-aware)
...
│   │   │   └── useUserPreferences.ts # Cross-device preferences (bankroll, initialBankroll, kelly, state — localStorage cache + DB sync)
...
- **Analysis Modal:** Click any prop card to see:
  ...
  - **AI Q&A chat** — collapsible "Ask AI about this pick" section powered by Claude Haiku. Multi-turn conversation grounded in player data. 8 data enrichment queries: (1) 10-game log with oreb/dreb/tov/fga/fta, (2) rolling averages L3/L5/L15/SZN, (3) player injury status, (4) player position, (5) opponent-specific matchup history (up to 5 games this season vs same team), (6) teammate injuries enriched with position + L15 averages, (7) opponent defense by position (expanded: raw + per100 + SZN + off_rtg), (8) opponent injuries enriched with position + L15 averages. Game context from model features: home/away, Vegas spread/total, team+opponent pace, expected pace, opponent defensive rating, starter probability, usage rate, minutes floor/variance. 20 questions/day rate limit. Suggested question chips on first open. Messages not persisted across modal close.
...
- **Schedule API** — `/api/games` server-side route fetches today's games. Accepts `sport` query param (`nba` default, `mlb`). NBA: fetches from `cdn.nba.com/static/json/staticData/scheduleLeagueV2.json`, maps tri-codes to full team names. MLB: fetches from `statsapi.mlb.com/api/v1/schedule`, filters cancelled/postponed games. Both use 1-hour revalidation cache. Used as fallback when predictions haven't been generated yet. Dashboard passes `sport` param from `SportContext`.
...
- **Under bet sanity checks (2026-03-23, improved 2026-03-25):** Two checks reject suspect under bets: (1) **Q50 divergence** — rejects when `pred_q50` is 30%+ below `feat_player_avg_stat_l5` (`MAX_Q50_DIVERGENCE = 0.30`). Uses `pd.notna()` guards to properly handle None/NaN. (2) **L5 above line** — rejects when L5 average >= prop line. Catches cases where the player is *averaging* above the line (e.g., Sexton L5=18 vs line=14.5). Both checks applied in `paper_trader.py` (bet selection) and `daily_runner.py` (BL recommendations). A `sanity_flag` TEXT column on `daily_predictions` persists warning reasons (e.g., `"l5_above_line:18.0>14.5"`) for all under-leaning predictions, enabling dashboard warnings.
...
**Dashboard insight features (G2/G3 — 2026-02-14, expanded 2026-03-25):** Added `feat_*` columns to `daily_predictions` table for template-based insights in the Analysis Modal and AI Q&A enrichment. Features explain WHY the model made its prediction.
1. **Database migration (G2):** Added columns for B2 rest/schedule (`feat_rest_days`, `feat_is_back_to_back`, `feat_games_last_7d`), B1 injury context (`feat_team_out_count`, `feat_team_out_min_sum`, `feat_opp_out_count`, `feat_player_is_questionable`, `feat_player_is_probable`), B3 stat-specific trends (`feat_player_avg_stat_l3/l5/l15`, `feat_stat_l3_l15_ratio`, `feat_stat_std_l5`), and opponent abbreviation (`feat_opp_abbrev`). B5 position-matched injury features (`feat_team_out_same_pos_count/min_sum/usg_sum/starter_sum`).
2. **Game context features (2026-03-25):** Added 11 `feat_*` columns for AI Q&A enrichment: `feat_line_spread`, `feat_line_total`, `feat_is_home`, `feat_player_starter_prob`, `feat_player_avg_usg_pct_l5`, `feat_team_avg_pace_l5`, `feat_opp_avg_pace_l5`, `feat_opp_avg_def_rtg_l5`, `feat_player_avg_min_l3`, `feat_player_min_floor_l5`, `feat_player_min_std_l5`. These features were already computed at inference time but not persisted — now stored for downstream use.
3. **Prediction storage update (G3):** `daily_runner.py` now maps feature values from `features_df` to predictions via `_map_features_to_predictions()` method. Uses hardcoded `TEAM_ABBREV` map (same as dashboard) for opponent abbreviations.
