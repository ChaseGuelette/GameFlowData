...
- **RPC Functions:** `get_dfs_lines` and `get_sportsbook_lines` scope by `commence_time::date` (not `daily_predictions`), enabling data availability before inference runs. `get_dfs_lines` joins with `players` table for `player_name` and returns `game_time` (commence_time). Migration `004_fix_rpc_prediction_dependency.sql`.
