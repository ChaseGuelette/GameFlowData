...
def compute_rolling_metrics(df):
    ...
    metrics = ['pts', 'reb', 'ast', 'threes', 'stl', 'blk', 'tov', 'fta', 'oreb', 'pf', 'poss_faced']
    ...
    for w_name, w_size in [('l5', 5), ('l15', 15)]:
        ...
        if isinstance(rolled.index, pd.MultiIndex): rolled = rolled.reset_index(drop=True)
        rolled.columns = [f"{c}_allowed_{w_name}" if c != 'poss_faced' else f"poss_faced_{w_name}" for c in rolled.columns]
        
        counts = grouper['pts'].apply(lambda x: x.shift(1).rolling(w_size, min_periods=1).count())
        if isinstance(counts.index, pd.MultiIndex): counts = counts.reset_index(drop=True)
        result_dfs.extend([rolled, counts.rename(f'games_{w_name}')])

    # Season Window
    szn = grouper[metrics].apply(lambda x: x.shift(1).expanding().sum())
    if isinstance(szn.index, pd.MultiIndex): szn = szn.reset_index(drop=True)
    ...
    if isinstance(szn_c.index, pd.MultiIndex): szn_c = szn_c.reset_index(drop=True)
    ...
    for w in ['l5', 'l15', 'szn']:
        ...
        for stat in base_stats:
            ...
            # Naming Exceptions
            if stat == 'tov': rate_col = f'tov_per100_forced_{w}'
            elif stat == 'pts': rate_col = f'off_rtg_allowed_{w}'
            else: rate_col = f'{stat}_per100_allowed_{w}'
...
def batch_insert_to_db(engine, df, batch_size=2000):
    valid_cols = [
        'team_id', 'game_id', 'game_date', 'position_group',
        'games_l5', 'games_l15', 'games_szn',
        'poss_faced_l5', 'poss_faced_l15', 'poss_faced_szn',
        # Core
        'pts_allowed_l5', 'pts_allowed_l15', 'pts_allowed_szn',
        'reb_allowed_l5', 'reb_allowed_l15', 'reb_allowed_szn',
        'ast_allowed_l5', 'ast_allowed_l15', 'ast_allowed_szn',
        'stl_allowed_l5', 'stl_allowed_l15', 'stl_allowed_szn',
        'blk_allowed_l5', 'blk_allowed_l15', 'blk_allowed_szn',
        'threes_allowed_l5', 'threes_allowed_l15', 'threes_allowed_szn',
        'tov_forced_l5', 'tov_forced_l15', 'tov_forced_szn',
        # Sharp
        'fta_allowed_l5', 'fta_allowed_l15', 'fta_allowed_szn',
        'oreb_allowed_l5', 'oreb_allowed_l15', 'oreb_allowed_szn',
        'pf_allowed_l5', 'pf_allowed_l15', 'pf_allowed_szn',
        
        # Core Rates
        'off_rtg_allowed_l5', 'off_rtg_allowed_l15', 'off_rtg_allowed_szn',
        'reb_per100_allowed_l5', 'reb_per100_allowed_l15', 'reb_per100_allowed_szn',
        'ast_per100_allowed_l5', 'ast_per100_allowed_l15', 'ast_per100_allowed_szn',
        'stl_per100_allowed_l5', 'stl_per100_allowed_l15', 'stl_per100_allowed_szn',
        'blk_per100_allowed_l5', 'blk_per100_allowed_l15', 'blk_per100_allowed_szn',
        'threes_per100_allowed_l5', 'threes_per100_allowed_l15', 'threes_per100_allowed_szn',
        'tov_per100_forced_l5', 'tov_per100_forced_l15', 'tov_per100_forced_szn',
        # Sharp Rates
        'fta_per100_allowed_l5', 'fta_per100_allowed_l15', 'fta_per100_allowed_szn',
        'oreb_per100_allowed_l5', 'oreb_per100_allowed_l15', 'oreb_per100_allowed_szn',
        'pf_per100_allowed_l5', 'pf_per100_allowed_l15', 'pf_per100_allowed_szn'
    ]
    rename_map = {'tov_allowed_l5': 'tov_forced_l5', 'tov_allowed_l15': 'tov_forced_l15', 'tov_allowed_szn': 'tov_forced_szn'}
    df = df.rename(columns=rename_map)[valid_cols].replace({np.nan: None})
    
    cols = ", ".join(valid_cols)
    vals = ", ".join([f":{c}" for c in valid_cols])
    upsert = text(f"INSERT INTO team_allowed_by_position ({cols}) VALUES ({vals}) ON CONFLICT (team_id, game_id, position_group) DO UPDATE SET " + ", ".join([f"{c} = EXCLUDED.{c}" for c in valid_cols if c not in ['team_id', 'game_id', 'position_group']]) + ", created_at = NOW();")
