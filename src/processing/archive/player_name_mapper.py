    ...
    with open(output_file, "w", newline="") as f:
        ...
        for api_name, count in unmatched:
            ...
            if matches:
                ...
                writer.writerow([api_name, count, best_id, best_name, f"{best_score:.2f}", other])
    ...
    while True:
        try:
            ...
            with engine.begin() as conn:
                conn.execute(
                    text(insert_query),
                    {"api_name": api_name, "player_id": player_id, "notes": notes},
                )
    ...
    with open(filepath) as f:
        ...
        for row in reader:
            ...
            with engine.begin() as conn:
                conn.execute(
                    text(insert_query),
                    {"api_name": api_name, "player_id": player_id, "notes": notes or None},
                )
            imported += 1
