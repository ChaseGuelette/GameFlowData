...
def main():
    ...
    parser.add_argument("--scrape-live-odds", action="store_true", help="Scrape LIVE odds (current lines) into raw_game_lines_live")
    parser.add_argument("--scrape-daily-props", action="store_true", help="Scrape daily player props (12pm/6pm) into raw_player_props_combined")
    ...
    if not args.skip_scraping:
        ...
        # Assuming current season logic or passing specific season
        # For simplicity, we run it without args which defaults to 2024-25
        run_command(
            "python src/scrapers/nba_unified_scraper.py",
            "Scraping Latest Game Results"
        )
        
        # Scrape Daily Game Lines (Odds)
        # We pass the target date to the scraper
        run_command(
    ...
    if not args.skip_inference:
        ...
        try:
            # Find latest run in artifacts dir if specific model path not provided
            # Or assume standard structure. The user provided --model-dir default.
            # We need to find the specific run folder inside artifacts if it exists
            
            # Simple logic: Look for 'run_*' folders, pick latest
            artifacts_path = Path(args.model_dir)
            if not artifacts_path.exists():
                raise FileNotFoundError(f"Model artifacts directory not found: {artifacts_path}")
            
            # Check if we are pointing to a specific run or the root
            ...
            # Production uses high sample count
