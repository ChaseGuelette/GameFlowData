...
class ESPNInjuryScraper:
    ...
    def fetch_current_injuries(self) -> dict | None:
        """
        ...
        if data:
            self.logger.info(f"Successfully fetched injury data at {datetime.now(UTC)}")
        else:
...
class InjuryChangeDetector:
    ...
    def detect_changes(
        self, previous_injuries: list[InjuryRecord], current_injuries: list[InjuryRecord]
        ...
        for injury_id in potential_updates:
            ...
            if (
                ...
                or prev.return_date != curr.return_date
                or prev.injury_type != curr.injury_type
            ):
