...
PARK_FACTORS = {
    ...
    # Last 6 values: L/R handedness split park factors (FanGraphs estimates, 2022-2025 avg)
    # L = left-handed batter factor, R = right-handed batter factor
    # Switch hitters: vs RHP use L factor, vs LHP use R factor
    15:   ("Chase Field",              1.06, 1.10, 1.03, 0.98,  1.06, 1.06, 1.12, 1.08, 1.04, 1.02),  # ARI
    2:    ("Fenway Park",              1.04, 0.95, 1.06, 0.97,  1.06, 1.02, 1.10, 0.82, 1.09, 1.03),  # BOS - Green Monster favors LHB
    4705: ("American Family Field",   1.02, 1.05, 1.01, 0.99,  1.02, 1.02, 1.08, 1.02, 1.01, 1.01),  # MIL
    17:   ("Wrigley Field",           1.05, 1.10, 1.03, 0.97,  1.05, 1.05, 1.12, 1.08, 1.04, 1.02),  # CHC
    4:    ("Guaranteed Rate Field",   1.01, 1.03, 1.00, 1.00,  1.01, 1.01, 1.04, 1.02, 1.00, 1.00),  # CWS
    27:   ("Great American Ball Park",1.08, 1.15, 1.04, 0.97,  1.08, 1.08, 1.18, 1.12, 1.05, 1.03),  # CIN
    5:    ("Progressive Field",       0.98, 0.95, 0.98, 1.01,  0.98, 0.98, 0.96, 0.94, 0.98, 0.98),  # CLE
    19:   ("Coors Field",             1.20, 1.15, 1.15, 0.85,  1.20, 1.20, 1.15, 1.15, 1.15, 1.15),  # COL - altitude equal
    2394: ("Comerica Park",           0.97, 0.92, 0.98, 1.02,  0.97, 0.97, 0.90, 0.94, 0.97, 0.99),  # DET
    2392: ("Minute Maid Park",        1.03, 1.05, 1.02, 0.99,  1.03, 1.03, 1.08, 1.02, 1.03, 1.01),  # HOU - Crawford Boxes favor LHB HR
    7:    ("Kauffman Stadium",        0.98, 0.95, 0.99, 1.01,  0.98, 0.98, 0.94, 0.96, 0.98, 0.99),  # KC
    1:    ("Angel Stadium",           0.97, 0.98, 0.98, 1.01,  0.97, 0.97, 0.97, 0.99, 0.97, 0.99),  # LAA
    22:   ("Dodger Stadium",          0.95, 0.92, 0.97, 1.02,  0.95, 0.95, 0.90, 0.94, 0.96, 0.98),  # LAD
    4169: ("loanDepot park",          0.96, 0.90, 0.97, 1.02,  0.96, 0.96, 0.88, 0.92, 0.96, 0.98),  # MIA
    3289: ("Citi Field",              0.96, 0.93, 0.97, 1.02,  0.96, 0.96, 0.91, 0.95, 0.96, 0.98),  # NYM
    3313: ("Yankee Stadium",          1.05, 1.12, 1.02, 0.98,  1.06, 1.04, 1.22, 1.04, 1.04, 1.00),  # NYY - short RF porch LHB HR
    10:   ("Oakland Coliseum",        0.93, 0.88, 0.95, 1.03,  0.93, 0.93, 0.87, 0.89, 0.94, 0.96),  # OAK
    2681: ("Citizens Bank Park",      1.04, 1.08, 1.02, 0.99,  1.04, 1.04, 1.10, 1.06, 1.03, 1.01),  # PHI
    31:   ("PNC Park",                0.96, 0.93, 0.97, 1.02,  0.96, 0.96, 0.92, 0.94, 0.97, 0.97),  # PIT
    2680: ("Petco Park",              0.94, 0.90, 0.96, 1.03,  0.94, 0.94, 0.89, 0.91, 0.95, 0.97),  # SD
    2395: ("Oracle Park",             0.92, 0.85, 0.95, 1.04,  0.92, 0.92, 0.83, 0.87, 0.94, 0.96),  # SF
    680:  ("T-Mobile Park",           0.95, 0.92, 0.97, 1.02,  0.95, 0.95, 0.91, 0.93, 0.96, 0.98),  # SEA
    2889: ("Busch Stadium",           0.97, 0.95, 0.98, 1.01,  0.97, 0.97, 0.94, 0.96, 0.97, 0.99),  # STL
    12:   ("Tropicana Field",         0.96, 0.93, 0.97, 1.02,  0.96, 0.96, 0.92, 0.94, 0.96, 0.98),  # TB - dome
    13:   ("Globe Life Field",        1.01, 1.03, 1.00, 1.00,  1.01, 1.01, 1.04, 1.02, 1.00, 1.00),  # TEX
    14:   ("Rogers Centre",           1.03, 1.05, 1.02, 0.99,  1.03, 1.03, 1.07, 1.03, 1.02, 1.02),  # TOR
    3309: ("Nationals Park",          1.00, 1.00, 1.00, 1.00,  1.00, 1.00, 1.00, 1.00, 1.00, 1.00),  # WSH
    2862: ("Target Field",            1.00, 1.02, 1.00, 1.00,  1.00, 1.00, 1.03, 1.01, 1.00, 1.00),  # MIN
    5325: ("Truist Park",             1.01, 1.03, 1.00, 1.00,  1.01, 1.01, 1.04, 1.02, 1.00, 1.00),  # ATL
}
...
def seed_park_factors(engine, seasons: list[int] | None = None):
    ...
    with engine.begin() as conn:
        for venue_id, vals in PARK_FACTORS.items():
            ...
            if len(vals) > 5:
                runs_l, runs_r, hr_l, hr_r, hits_l, hits_r = vals[5:]
            else:
                runs_l, runs_r, hr_l, hr_r, hits_l, hits_r = runs, runs, hr, hr, hits, hits
