"""
MLB Daily Game Lines Scraper
=============================
Scrapes MLB game lines (moneyline, spreads, totals) from The Odds API.
Stores into mlb_raw_game_lines table.

Pattern: src/scrapers/daily_game_lines_scraper.py + game_lines_scraper.py (NBA equivalents)
Change: sport key basketball_nba -> baseball_mlb, table -> mlb_raw_game_lines

Supports both live and historical scraping modes.

Usage:
    python -m src.scrapers.mlb.mlb_daily_game_lines_scraper --live
    python -m src.scrapers.mlb.mlb_daily_game_lines_scraper --date 2025-04-01
...
logger = logging.getLogger("MLBGameLines")
...
class MLBGameLineScraper:
    ...
    def get_live_odds(self):
        """Fetch live game lines for all upcoming MLB games.
    ...
    def parse_and_store(self, response_json, snapshot_ts):
        """Parse game lines response and insert into mlb_raw_game_lines."""
        # Live endpoint returns list directly; historical wraps in 'data'
    ...
    def _batch_insert(self, rows):
        ...
        try:
            with conn.cursor() as cur:
                query = """
                    INSERT INTO mlb_raw_game_lines
...
def run_live_scrape(scraper):
    """Scrape live MLB game lines."""
    logger.info("Running LIVE MLB Game Lines Scrape...")
...
def run_date_scrape(scraper, date_str):
    """Scrape historical MLB game lines for a specific date."""
    logger.info(f"Scraping MLB game lines for {date_str}...")
    ...
    # MLB games span afternoon + evening
    # 17:00 UTC (12pm ET), 20:00 UTC (3pm ET), 23:00 UTC (6pm ET)
    total_rows = 0
    total_credits = 0
...
def run_backfill(scraper, start_date: str, end_date: str):
    """Backfill MLB game lines over a date range."""
    logger.info(f"Backfilling MLB game lines from {start_date} to {end_date}...")
...
if __name__ == "__main__":
    ...
    scraper = MLBGameLineScraper(API_KEY, engine)

    parser = argparse.ArgumentParser(description="MLB Daily Game Lines Scraper")
