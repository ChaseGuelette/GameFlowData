...
class InjuryScraperJob:
    ...
    def run(self) -> dict:
        ...
        results = {"success": False, "stats": {}, "errors": []}
        ...
        try:
            ...
            if previous_injuries_data:
                ...
                self.logger.info(
                    f"Previous count: {len(previous_injuries_data)}, Current count: {len(current_injuries)}"
                )

            stats = {"total_injuries": len(current_injuries), "inserted": inserted}
            results["stats"] = stats
