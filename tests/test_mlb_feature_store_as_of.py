...
def test_pitcher_feature_store_prop_line_query_applies_as_of_cutoff(monkeypatch):
    ...
    assert "ORDER BY COALESCE(snapshot_time, inserted_at) DESC NULLS LAST" in captured["sql"]
...
def test_pitcher_batch_feature_query_applies_as_of_cutoff(monkeypatch):
    ...
    assert result.empty
    assert "COALESCE(snapshot_time, inserted_at) <= :as_of_time" in captured["sql"]
    assert "ORDER BY market_key, COALESCE(snapshot_time, inserted_at) DESC NULLS LAST" in captured["sql"]
    assert captured["params"]["as_of_time"] == as_of
...
def test_quote_clean_line_fetch_uses_effective_timestamp_fallback(monkeypatch):
    ...
    assert result.empty
    assert "COALESCE(snapshot_time, inserted_at) <= :quote_clean_cutoff_ts" in captured["sql"]
    ...
    assert captured["params"]["quote_clean_cutoff_ts"] == as_of
